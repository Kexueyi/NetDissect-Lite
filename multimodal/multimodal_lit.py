...
class MultiModalLitModel(pl.LightningModule):
    ...
    def __init__(self, vision_encoder, text_encoder, args):
        ...
        self.model_device = torch.device('cuda' if torch.cuda.is_available() else 'cpu') # modi
    ...
    def forward(self, x, y=None, y_len=None):
        ...
        if y is None:
            # 如果y为None，传递空的LongTensor给text和text_length
            text = torch.tensor([], dtype=torch.long).to(self.model_device)
            text_length = torch.tensor([]).to(self.model_device)
            return self.model(x, text, text_length)
        else:
            # 如果y不为None，调用self.model的forward方法
            y = y.to(self.model_device)
            y_len = y_len.to(self.model_device)
