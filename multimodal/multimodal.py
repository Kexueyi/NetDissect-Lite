...
class TextEncoder(nn.Module):
            ...
            for i, i_len in zip(x, x_len):
                ...
                if self.embedding_type == "flat":
                    ...
                    padded_output = torch.zeros(
                        (max_seq_len, self.embedding_dim)).to(self.model_device)
    ...
    def init_hidden(self, batch_size, image_features=None):
        ...
        return (torch.zeros(d * self.lstm.num_layers, batch_size, self.hidden_dim).to(self.model_device),
                torch.zeros(d * self.lstm.num_layers, batch_size, self.hidden_dim).to(self.model_device))
...
class MultiModalModel(nn.Module):
    ...
    def forward(self, image, text=None, text_length=None, process_text=True, return_image_features=False, return_text_outputs=False):
        # encode image
        image_features, image_feature_map = self.encode_image(image)

        if process_text and text is not None and text_length is not None:
            # 当决定处理文本且文本数据有效时，进行文本编码
            text_features, text_outputs = self.encode_text(text, text_length)
        else:
            # 不处理文本时，使用默认的文本特征占位
            # 注意：根据具体模型的需要可能要调整默认值
            # 这里假设text_features的维度与image_features第二维匹配
            batch_size = image_features.size(0)
            if self.embedding_type == "flat":
                # 对于flat类型的embedding，需要保证维度匹配
                text_feature_dim = image_features.size(-1)  # 假设文本特征维度与图像特征最后一维相同
                text_features = torch.zeros(batch_size, text_feature_dim, device=image.device)
            elif self.embedding_type == "spatial":
                # 对于spatial类型的embedding，维度可能需要不同的处理方式
                # 此处假设为简化处理，具体根据模型需求调整
                text_feature_dim = image_features.size(1)  # 假设与图像特征第二维相同
                text_features = torch.zeros(batch_size, text_feature_dim, device=image.device)
            text_outputs = None  # 或相应的默认值

        match = None  # 初始化match变量以确保在下面的逻辑中使用

        if self.embedding_type == "flat":
            # image_features: (B, E), text_features: (B, E)
            match = image_features @ text_features.T
        elif self.embedding_type == "spatial":
            # 对于spatial类型的embedding，您可能需要根据text_features是否为默认值进行特殊处理
            # 此处保持您原始的逻辑不变
            if self.sim == "mean":
                match_sum = torch.einsum('iehw,tle->it', [image_features, text_features])
                match = match_sum / (image_features.size(-2) * image_features.size(-1) * text_length)
            elif self.sim == "max":
